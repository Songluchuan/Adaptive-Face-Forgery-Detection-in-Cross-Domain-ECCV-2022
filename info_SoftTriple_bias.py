...
class SoftTriple_bias(nn.Module):
    def __init__(self, la, gamma, tau, margin, dim, cN, K, pl = 0,
        ...
        self.positive_weight = torch.tensor(positive_weight).float()
        ...
        self.weight = torch.zeros(sum(self.K), sum(self.K), dtype=torch.bool)
...
if __name__=="__main__":
    model = SoftTriple_bias(20, 10, 0, 0, 128, 2, 1, ori_dim=4352)
    model.eval()
    # params = list(model.named_parameters())
    # for name, param in params:
    #     print(name)
    from thop import profile
    input = torch.randn(1, 4352)
    target = torch.zeros(1).long()
    bias = torch.randn(1, 256)
    macs, params = profile(model, inputs=(input, target, bias))
    print(macs, params)
